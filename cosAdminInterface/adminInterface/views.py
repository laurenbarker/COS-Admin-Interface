from database import get_all_drafts, get_schema, get_draft, get_draft_obj, get_file_data
...
def password_reset_done(request, **kwargs):
    messages.success(
        request,
        'You have successfully reset your password and activated your admin account. Thank you'
    )
...
def get_drafts(request):
    ...
    return HttpResponse(
        json.dumps(all_drafts), content_type='application/json'
    )
...
def view_file(request, file_name):
    ...
    url = get_file_data(file_name)

    context = {
        'mfr_path': mfr_path('utils.py'),
